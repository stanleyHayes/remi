...
export async function DELETE(_: Request, context: { params: Promise<{ delegationId: string }> }) {
  ...
  const response = await fetch(apiURL(`/api/member/household-delegations/${encodeURIComponent(delegationId)}`), { method: "DELETE", headers: { Authorization: `Bearer ${access}` }, cache: "no-store" });
