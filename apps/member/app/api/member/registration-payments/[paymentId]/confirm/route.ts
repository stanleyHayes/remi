...
export async function POST(_: Request, { params }: { params: Promise<{ paymentId: string }> }) {
  ...
  const response = await fetch(apiURL(`/api/member/registration-payments/${encodeURIComponent(paymentId)}/confirm`), { method: "POST", headers: { Authorization: `Bearer ${access}` }, cache: "no-store" });
