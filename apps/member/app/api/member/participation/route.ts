...
export async function GET() { const access=(await cookies()).get("remi_member_access")?.value;if(!access)return NextResponse.json({error:"Sign in again."},{status:401});const response=await fetch(apiURL("/api/member/participation"),{headers:{Authorization:`Bearer ${access}`},cache:"no-store"});return NextResponse.json(await parseResponse(response),{status:response.status}); }
